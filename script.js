document.addEventListener("DOMContentLoaded", () => {
    ...
    if (typeof round1Questions !== 'undefined') {
        const moderateQuestions = round1Questions.filter(q => q.difficulty === 'moderate');
        const hardQuestions = round1Questions.filter(q => q.difficulty === 'hard');
        const selectedModerate = moderateQuestions.sort(() => Math.random() - 0.5).slice(0, 4);
        const selectedHard = hardQuestions.sort(() => Math.random() - 0.5).slice(0, 5);
        questions = [...selectedModerate, ...selectedHard].sort(() => Math.random() - 0.5);
        console.log(`Loaded ${questions.length} questions: ${selectedModerate.length} Moderate, ${selectedHard.length} Hard.`);
    } else {
